...
def add_iface_stats(device_name: str, iface_name: str) -> None:
    db.stats.insert_one({'device_name': f"{device_name}", 
        ...
        'in_errors': 0, 'out_discards': 0, 'out_errors': 0, 'in_bytes': 0,
        'in_ucast_pkts': 0, 'in_mcast_pkts': 0, 'in_bcast_pkts': 0,
        'out_bytes': 0, 'out_ucast_pkts': 0, 'out_mcast_pkts': 0, 'out_bcast_pkts': 0}
        )

def add_static_node(node_name: str, node_ip: str, node_ifaces: List[str], neigh_infos: List[Tuple[str,str,str]]) -> None:
    ...
    if neigh_infos:
        for neigh in neigh_infos:
            ...
            db.links.insert_one({'device_name': node_name,
            'iface_name': local_iface, 'neighbor_iface': neigh_iface, 'neighbor_name': neigh_name})

            db.links.insert_one({'device_name': neigh_name,
            'iface_name': neigh_iface, 'neighbor_iface': local_iface, 'neighbor_name': node_name})
...
def main():
    ...
    if not args.ifaces_of_node:
        while ( res:= input("This node has no ifaces. Are you sure you wanna add a 'free' node with no links ? (Enter y/n)\n").lower().strip() ) not in {"y", "n"}: pass
        if res == "n":
            print("Ok, bye!")
            sexit(1)
    else:
        ...
        for iface in args.ifaces_of_node.split(','):
            ...
            if res == "n":
                continue
            neigh_fqdn = input(f"Please enter the fqdn of the neighbor of the iface {iface} (just hit ENTER if no fqdn)\n").lower()
            neigh_ip = input(f"Please enter the ip of the neighbor of the iface {iface}\n").lower()
            neigh_iface = input(f"Please enter the iface (like 3/1)  of the neighbor of the local iface {iface}\n").lower()
