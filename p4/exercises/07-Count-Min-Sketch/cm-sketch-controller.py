...
class CMSController(object):
    ...
    def read_registers(self):
        for counter in range(self.register_num):
            self.registers.append(self.controller.register_read('sketch' + str(counter)))
    ...
    def get_cms(self, flow, mod):
        ...
        for counter in range(self.register_num):
            index = self.hashes[counter].bit_by_bit_fast((self.flow_to_bytestream(flow))) % mod
            #print(index, counter)
            values.append(self.registers[counter][index])
        return min(values)

    #TODO 3: implement the main function that decodes all the registers, and compares them to the ground truth
    def decode_registers(self, epsilon=0.01, nb_packets=1000, reg_length=4096, flows_file="sent_flows.pickle"):
        self.read_registers()
        flows_inside_bound = 0
        flows = pickle.load(open(flows_file, 'r'))
        for flow, packet_count in flows.items():
            cms = self.get_cms(flow, reg_length)
            #if cms >= (packet_count + (epsilon * nb_packets)):
            if not (cms <(packet_count + (epsilon * nb_packets))):
                flows_inside_bound = flows_inside_bound + 1

        print("Inside the bounds : {}%".format(float(flows_inside_bound)/len(flows)*100))
