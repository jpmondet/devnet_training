...
control MyIngress(inout headers hdr,
    ...
    action mac_learn(){
        meta.ingr_port = standard_metadata.ingress_port;
...
control MyEgress(inout headers hdr,
    ...
    apply {
        ...
        // header and fill its fields. Finally set the ethernet type to 
        // L2_LEARN_ETHER_TYPE (defined above).
        if (standard_metadata.instance_type == 1) {
            hdr.cpu_hdr.setValid();
            hdr.cpu_hdr.srcAddr = hdr.ethernet.srcAddr;
            hdr.cpu_hdr.ingr_port = (bit<16>)meta.ingr_port;
