...
class RoutingController(object):
    ...
    def set_egress_type_table(self):
        for sw_name, controller in self.controllers.items():
            for intf, host in self.topo.get_interfaces_to_node(sw_name).items():
                sw_port = self.topo.interface_to_port(sw_name, intf)
                if host[0] == 'h':
                    device = 1
                else:
                    device = 2

                print("table_add on {}:".format(sw_name))
                print("table_add attached_device.set_next_device {}:{}:".format(sw_port,device))
                controller.table_add("attached_device", "set_next_device", [str(sw_port)], [str(device)])



    def add_mirroring_ids(self):
        for _, controller in self.controllers.items():
            controller.mirroring_add(64, 1)
