...
control MyIngress(inout headers hdr,
    ...
    register<bit<32>>(1024) flow_register;

    action update_flow_register(){

      bit<12> flow_id;
      bit<32> new_id;
      
      random(new_id, (bit<32>)0, (bit<32>)777777);
      ...
      flow_register.write((bit<32>)flow_id, new_id);
    }
    ...
    action ecmp_group(bit<14> ecmp_group_id, bit<16> num_nhops){
      ...
      flow_register.read(new_id, (bit<32>)flow_id);
    ...
    apply {
        ...
        if (hdr.ethernet.etherType == 0x7778){
          if (meta.device != 2) {
            update_flow_register();
            drop();
          }
        }
